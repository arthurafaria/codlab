...
export function findSections(text, keys = []) {
  const lines = String(text).replace(/\r\n?/g, "\n").split("\n");
  const conhecidas = new Set(keys.map((k) => canon(k)));
  const out = [];
  lines.forEach((raw, index) => {
    ...
    // Numeração sozinha não faz seção: "1. Leia o material inteiro" é
    // instrução, não variável. Precisa ter underline ou ser uma coluna
    // conhecida da planilha.
    const pareceVariavel =
      label.includes("_") || conhecidas.has(canon(label)) || conhecidas.has(canon(splitRunOn(label).label));
    if (!pareceVariavel) return;
...
export function parseCodebookDoc(text, keys = [], aliases = {}) {
  ...
  const sections = findSections(text, keys);
  ...
  lines.forEach((raw, index) => {
    ...
    for (const { needle, key } of needles) {
      ...
      // "Tipo_URL" não pode abocanhar a seção de "Tipo_URL_2": número logo
      // depois do nome indica outra variável, não a pergunta dela.
      if (/^\d+\b/.test(canon(rest))) continue;
  ...
  // Prefixo diferente, resto igual: Funcao_Panico e Efeito_Panico são a mesma
  // variável renomeada. Só liga quando houver uma única candidata, para não
  // trocar duas colunas parecidas uma pela outra.
  const sufixo = (nome) => {
    const partes = canon(nome).split(" ");
    return partes.length > 1 ? partes.slice(1).join(" ") : "";
  };
  const livres = sections.filter((s) => !marks.some((m) => m.index === s.index));
  for (const key of keys) {
    if (usados.has(key)) continue;
    const alvo = sufixo(key);
    if (!alvo || alvo.length < 4) continue;
    const candidatas = livres.filter((s) => sufixo(s.label) === alvo);
    if (candidatas.length !== 1) continue;
    const sec = candidatas[0];
    if (marks.some((m) => m.index === sec.index)) continue;
    marks.push({ index: sec.index, key, rest: sec.rest, auto: true });
    usados.add(key);
  }
