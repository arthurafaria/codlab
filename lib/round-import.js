...
function inferType(values, column = "") {
  ...
  const name = column.trim().toLowerCase();
  if (FREE_TEXT_HINTS.some((h) => name === h || name.startsWith(`${h}_`))) {
    return { type: "text", options: [] };
  }
  if (!distinct.length) return { type: "text", options: [] };
  ...
  // Duas ou mais respostas repetidas e curtas indicam lista fechada. Uma só
  // resposta observada não prova nada: nesse caso fica texto livre.
  if (distinct.length >= 2 && distinct.length <= 15 && distinct.every((v) => v.length <= 60)) {
...
export function parseRoundWorkbook(buffer, { fileName = "rodada", sheetName = "" } = {}) {
  const workbook = XLSX.read(buffer, { type: "array" });
  ...
  if (!variableRows || !itemRows) {
    return buildInferredRound(workbook, { fileName, sheetName });
  }
...
function buildInferredRound(workbook, { fileName, sheetName }) {
  ...
  return {
    ...
    records,
    codebook: {
      editableFields: inferredCodebook.editableFields,
    ...
    summary: {
      fileName,
      items: records.length,
      variables: inferredCodebook.editableFields.length,
      codable: inferredCodebook.editableFields.length,
      textField: inferredCodebook.textField,
      groups: [...new Set(inferredCodebook.editableFields.map((f) => f.group))],
      inferred: true,
      sheet: chosen.name,
