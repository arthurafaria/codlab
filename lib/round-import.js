...
export function parseRoundWorkbook(buffer, { fileName = "rodada" } = {}) {
  ...
  if (missing.length) {
    throw new Error(
      `A planilha precisa das abas ${missing.map((m) => `"${m}"`).join(" e ")}. ` +
        `Encontrei: ${workbook.SheetNames.map((s) => `"${s}"`).join(", ")}.`,
    );
  }
  ...
  if (!editableFields.length) {
    throw new Error('A aba "variables" não tem nenhuma linha com `variable_key` preenchido.');
  }

  const items = itemRows.filter((row) => Object.values(row).some((v) => norm(v)));
  if (!items.length) {
    throw new Error('A aba "items" está vazia.');
  }

  const textField = pickTextColumn(items);
  if (!textField) {
    throw new Error(
      'Não achei a coluna com o material a codificar na aba "items". ' +
        "Nomeie uma coluna como `texto` (ou text, mensagem, conteudo, prompt).",
    );
  }
  ...
  return {
    project: {
      id: roundId,
      eyebrow: "Rodada carregada neste navegador",
      title: title || "Rodada sem título",
    ...
    codebook: {
      ...
      defaultBinaryFormat: "FALSE/TRUE",
    },
    summary: {
      fileName,
