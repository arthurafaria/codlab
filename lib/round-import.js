...
// A mesma planilha era decodificada duas vezes: uma para listar as abas, outra
// para abrir. Em arquivo grande isso trava a interface sem necessidade.
const workbookCache = new WeakMap();

function readWorkbook(buffer) {
  const chave = buffer?.buffer instanceof ArrayBuffer ? buffer.buffer : buffer;
  if (chave && typeof chave === "object") {
    const cached = workbookCache.get(chave);
    if (cached) return cached;
    const wb = XLSX.read(buffer, { type: "array" });
    workbookCache.set(chave, wb);
    return wb;
  }
...
export function parseRoundWorkbook(buffer, { fileName = "rodada", sheetName = "", codebookDoc = null, aliases = {} } = {}) {
  const workbook = readWorkbook(buffer);
  ...
  if (!variableRows || !itemRows) {
    return buildInferredRound(workbook, { fileName, sheetName, codebookDoc, aliases });
  }
  ...
  // A ordem real das colunas da aba vai junto: é ela que decide onde o bloco
  // copiado deve cair, não a ordem declarada em output_order.
  const sheetOrder = firstVariableColumn >= 0 ? itemColumns : null;
  ...
  const doc = codebookDoc ? parseCodebookDoc(codebookDoc, editableFields.map((f) => f.key), aliases) : null;
  ...
  return {
    ...
    codebookDoc: doc
      ? { text: doc.text, matched: doc.matched, unmatched: doc.unmatched, freeSections: doc.freeSections }
      : null,
    summary: {
      fileName,
...
function buildInferredRound(workbook, { fileName, sheetName, codebookDoc = null, aliases = {} }) {
  ...
  const doc = codebookDoc
    ? parseCodebookDoc(codebookDoc, inferredCodebook.editableFields.map((f) => f.key), aliases)
    : null;
...
export function saveRound(round) {
  ...
  const mantidas = [entry, ...index].slice(0, 12);
  // Sair da lista tem que liberar o espaço: antes a rodada sumia da tela e
  // continuava ocupando o armazenamento, sem jeito de recuperar nem de apagar.
  const mantidasIds = new Set(mantidas.map((r) => r.id));
  for (const antiga of index) {
    if (mantidasIds.has(antiga.id)) continue;
    localStorage.removeItem(`codlab:def:${antiga.id}`);
    if (antiga.storageKey) localStorage.removeItem(antiga.storageKey);
  }
  localStorage.setItem(ROUND_INDEX_KEY, JSON.stringify(mantidas));
  return entry;
}
